...
"""Print configuration and image compliance for inventory devices."""
...
def _get_status(client, serial, hostname):
    return serial, hostname, client.get_device_status(serial)


def _print_status(serial, hostname, status):
    config = status.get("config_summary", {})
    image = status.get("image_status", {})
    error = status.get("error") or config.get("error") or image.get("error") or ""
    if error:
        print(f"{serial}: {error}", file=sys.stderr)
    dual_sup = image.get("dual_sup", False)
    peer_image = (
        _compliance_text(image.get("image_compliance_for_peer_sup"), error)
        if dual_sup
        else "n/a"
    )
    peer_extensions = (
        _compliance_text(image.get("extension_compliance_for_peer_sup"), error)
        if dual_sup
        else "n/a"
    )
    print(
        f"{hostname:<25} {serial:<32} "
        f"{_compliance_text(config.get('config_compliance'), error):<10} "
        f"{_compliance_text(image.get('image_compliance'), error):<10} "
        f"{_compliance_text(image.get('extension_compliance'), error):<12} "
        f"{peer_image:<12} {peer_extensions:<16}"
    )
...
async def main(args):
    ...
    print(
        f"{'Hostname':<25} {'Device serial':<32} {'Config':<10} {'Image':<10} "
        f"{'Extensions':<12} {'Peer image':<12} {'Peer extensions':<16}"
    )
    with ComplianceClient(compliance_target, token, cert=cert) as client:
        loop = asyncio.get_running_loop()
        with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            futures = [
                loop.run_in_executor(executor, _get_status, client, serial, hostname)
                for serial, hostname in devices
            ]
            for future in asyncio.as_completed(futures):
                serial, hostname, status = await future
                _print_status(serial, hostname, status)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Get configuration and image compliance for devices in inventory.")
    )
    parser.add_argument(
        "--server",
        required=True,
