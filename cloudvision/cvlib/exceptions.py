...
class InputError:
    ...
    def __init__(self, message: str = "Error in input field",
        ...
        #: A user-friendly text message of the error
        self.message = message

        self.inputPath = inputPath
        '''
        The path to the field that is in error. It is a list of field names
        (the "name" attribute in the schema) starting from the root.
        E.g.: ``["networkConfig", "0", "config", "monitoredHosts", "1"]``
        '''

        #: The unique ID of the field (the "id" attribute in the schema).
        self.fieldId = fieldId

        self.members = members
        '''
        A list of all members in a group-type input that are in conflict. inputs easily.
